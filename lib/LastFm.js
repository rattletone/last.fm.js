...
class LastFm {
	 ...
	 * Search for an album
	 * @param {Object} params
	 * @param {Object} [params.all] - Item to search for. Required unless params.album, params.artist, and params.track are set.
	 * @param {string} [params.album] - Album to search for
	 * @param {string} [params.artist] - Artist, and whose track to search for
	 * @param {string} [params.track] - Track to search for
	 * @param {number} [params.limit] - Number of items to get per page
	 * @param {number} [params.page] - Page number to get
	 * @param {callback} [callback]
	...
	search(params, callback) {
		const searchAll = (params, callback) => {
			Promise.all([
				this.albumSearch(params),
				this.artistSearch(params),
				this.trackSearch(params)
			])
			.then(dataArr => {
				const [album, artist, track] = dataArr;

				callback(null, { album, artist, track });
			})
			.catch(err => {
				callback(err, null);
			});
		...
		if(params.all) {
			parsedParams.album = params.album || params.all;
			parsedParams.artist = params.artist || params.all;
			parsedParams.track = params.track || params.all;
		}

		if(typeof callback === "function") {
			searchAll(parsedParams, callback);

			return this;
		}

		return new Promise((resolve, reject) => searchAll(parsedParams, (err, data) => {
			if(err) {
				reject(err);
